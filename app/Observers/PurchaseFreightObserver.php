...
class PurchaseFreightObserver
{
    ...
    /**
     * Handle the PurchaseFreight "creating" event.
     */
    public function creating(PurchaseFreight $freight)
    {
        $this->handleFileUploads($freight);
    }

    /**
     * Handle the PurchaseFreight "updating" event.
     */
    public function updating(PurchaseFreight $freight)
    {
        $this->handleFileUploads($freight);
    }

    /**
     * Handle all file uploads for the PurchaseFreight model
     */
    protected function handleFileUploads(PurchaseFreight $freight)
    {
        $fileFields = [
            'bilty_slip',
            'weighbridge_slip',
    ...
    protected function optimizeAndStoreImage($file, $fieldName, $extension)
    {
        ...
        $filename = 'purchase_freight-' . $fieldName . '-' . $sluggedName . '-' . now()->format('YmdHis') . '.' . $extension;
    ...
    protected function storeFreightFile($file, $fieldName)
    {
        $originalName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
        $sluggedName = Str::slug($originalName);
        $filename = 'purchase_freight-' . $fieldName . '-' . $sluggedName . '-' . now()->format('YmdHis') . '.' . $file->getClientOriginalExtension();
    ...
    /**
     * Handle the PurchaseFreight "deleted" event.
     */
    public function deleted(PurchaseFreight $freight)
    {
        $fileFields = [
            'bilty_slip',
            'weighbridge_slip',
