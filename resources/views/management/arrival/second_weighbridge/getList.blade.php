        ...
        @if (count($ArrivalSamplingRequests) != 0)
            @foreach ($ArrivalSamplingRequests as $key => $row)
                <tr>
                    ...
                    <td>
                        {{-- @can('role-edit') --}}
                        <a onclick="openModal(this,'{{ route('second-weighbridge.edit', $row->id) }}','View Second Weighbridge', true)"
                            class="info p-1 text-center mr-2 position-relative ">
                            <i class="ft-eye font-medium-3"></i>
                        </a>
                        {{-- @endcan
                        @can('role-delete')
                            <a onclick="deletemodal('{{ route('location-transfer.destroy', $row->id) }}','{{ route('get.second-weighbridge') }}')"
