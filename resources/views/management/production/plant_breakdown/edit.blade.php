<form method="POST" action="{{route('plant-breakdown.update', $plantBreakdown->id)}}" id="ajaxSubmit" autocomplete="off">
    @method('PUT')
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Date:</label>
                        <input type="date" name="date" class="form-control" value="{{ $plantBreakdown->date->format('Y-m-d') }}" required>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="plant_id" id="plant_id" class="form-control select2" required>
                            ...
                            @foreach($plants as $plant)
                                <option value="{{ $plant->id }}" {{  $plantBreakdown->plant_id == $plant->id ? 'selected' : '' }}>
                ...
                <div class="col-md-12">
                    <div id="breakdownItemsTable">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @if($plantBreakdown->items->count() > 0)
                                    @foreach($plantBreakdown->items as $item)
                                        <tr>
                                            <td>
                                                <select name="breakdown_type_id[]" class="form-control " required>
                                                    ...
                                                    @foreach($breakdownTypes as $type)
                                                        <option value="{{ $type->id }}" {{ $item->breakdown_type_id == $type->id ? 'selected' : '' }}>
                                                            {{ $type->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <textarea name="remarks[]" class="form-control" rows="1">{{ $item->remarks }}</textarea>
                                            </td>
                                            <td>
                                                <button type="button" class="btn btn-sm btn-primary copythis"><i class="fa fa-plus"></i> Add more</button>
                                                <button type="button" class="btn btn-sm btn-danger removethis"><i class="fa fa-trash"></i> Remove</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td>
                                            <select name="breakdown_type_id[]" class="form-control select2" required>
                                                <option value="">Select Breakdown Type</option>
                                                @foreach($breakdownTypes as $type)
    ...
    <div class="row bottom-button-bar text-right">
        <div class="col-12">
            ...
            <button type="submit" class="btn btn-primary submitbutton">Update Plant Breakdown</button>
        </div>
    </div>
</form>
