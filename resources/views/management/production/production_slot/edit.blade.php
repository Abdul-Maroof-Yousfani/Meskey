<form action="{{ route('production-slot.update', $productionSlot->id) }}" method="POST" id="ajaxSubmit" autocomplete="off">
    @csrf
    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="production_voucher_id" id="production_voucher_id" class="form-control select2" required>
                            ...
                            @foreach($productionVouchers as $voucher)
                                <option value="{{ $voucher->id }}" {{ $productionSlot->production_voucher_id == $voucher->id ? 'selected' : '' }}>
                                    {{ $voucher->prod_no }} - {{ $voucher->prod_date->format('Y-m-d') }}
                                </option>
                            @endforeach
                        </select>
                    </div>
