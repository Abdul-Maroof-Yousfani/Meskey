        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="ajaxSubmit" action="{{ route('payment-voucher.bill-update', [ 'id' => $payment_voucher->id ]) }}">
                            @csrf
                            {{ method_field("PUT") }}
                            ...
                            <input type="hidden" name="bank_account_type" id="bank_account_type" value="0">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="voucher_type" id="voucher_type" class="form-control select2" required>
                                            ...
                                            <option value="bank_payment_voucher" @selected($payment_voucher->voucher_type == 'bank_payment_voucher')>Bank Payment Voucher</option>
                                            @endcanAccess
                                            @canAccess('cash-payment-voucher')
                                            <option value="cash_payment_voucher" @selected($payment_voucher->voucher_type == 'cash_payment_voucher')>Cash Payment Voucher</option>
                                            @endcanAccess
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="pv_date" id="pv_date" value="{{ \Carbon\Carbon::parse($payment_voucher->pv_date)->format('Y-m-d') }}" class="form-control" required>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="bank_account_id" id="account_id" class="form-control select2" required>
                                            <option value="">Select Account</option>
                                            @foreach ($bank_accounts as $account)
                                            <option value="{{ $account->id }}" @selected($account->id == $payment_voucher->bank_account_id)>{{ $account->name }}
                                                ({{ $account->unique_no }})
                                            </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="ref_bill_no" value="{{ $payment_voucher->ref_bill_no }}" id="ref_bill_no" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="bill_date">Bill Date</label>
                                        <input type="date" name="bill_date" id="bill_date" value="{{ \Carbon\Carbon::parse($payment_voucher->bill_date)->format('Y-m-d') }}" class="form-control">
                                    </div>
                                </div>
                            </div>

                            
                            <div class="row bank-fields" @if($payment_voucher->voucher_type != 'bank_payment_voucher') style="display: none" @endif>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="cheque_no">Cheque No.</label>
                                        <input type="text" name="cheque_no" value="{{ $payment_voucher->cheque_no }}" id="cheque_no" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="cheque_date">Cheque Date</label>
                                        <input type="date" name="cheque_date" value="{{ \Carbon\Carbon::parse($payment_voucher->cheque_date)->format('Y-m-d') }}" id="cheque_date" class="form-control">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <input type="hidden" name="purchase_bill_ids" value="" id="purchase_bill_id">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="req_account_id">Supplier Account</label>
                                        <select name="supplier_id" id="req_account_id" class="form-control select2">
                                            <option value="">Select Account</option>
                                            @foreach ($suppliers as $supplier)
                                                <option value="{{ $supplier->id }}" @selected($supplier->id == $payment_voucher->supplier_id)>{{ $supplier->name }}
                                                    ({{ $supplier->unique_no }})
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="payment-requests-container">
                                            <div class="table-responsive">
                                                <table class="table table-bordered" id="paymentRequestsTable">
                                                    ...
                                                    <tbody>
                                                        @foreach($payment_voucher_data as $data)
                                                            <tr>
                                                                <td>
                                                                    <input type="checkbox" class="form-control item-checkbox"  data-amount="{{ $data->amount }}" value="{{ $data->purchase_bill_id }}" data-transaction="{{ purchase_bill($data->purchase_bill_id)->bill_no }}" name="bill[]" style="width: 12px; height: 12px; margin: 0 auto;" checked/>
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control" name="bill_no[]" value="{{ purchase_bill($data->purchase_bill_id)->bill_no }}" readonly/>
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control" name="grn_no[]" value="{{ purchase_bill($data->purchase_bill_id)->grn->reference_no }}" readonly />
                                                                </td>
                                                                <td>
                                                                    <input type="text" class="form-control" name="supplier[]" value="{{ purchase_bill($data->purchase_bill_id)->supplier->name }}" readonly />
                                                                </td>
                                                                <td>
                                                                    <input type="hidden" name="purchase_bill_id[]" value="{{ $data->purchase_bill_id }}" />
                                                                    <input type="text" class="form-control amount-field" name="amounts[]" value="{{ $data->amount }}" />
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Selected Purchase Bills</label>
                                        <div class="selected-requests-container bg-light p-3">
                                            <p class="text-muted">No purchase bills are selected yet</p>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="remarks" id="remarks" class="form-control" rows="3">{{ $payment_voucher->remarks }}</textarea>
                                    </div>
                                </div>
                            </div>
        ...
        $(document).ready(function () {
            ...
            $('#req_account_id').change(function () {
                ...
                if (accountId) {
                    ...
                    $.ajax({
                        url: `/finance/payment-voucher/purchase-bills/${accountId}`,
                        type: 'GET',
                        success: function (response) {
                            tbody.empty();
            ...
            function updateSelectedRequestsList() {
                ...
                $('.item-checkbox:checked').each(function () {
                    selectedBills.push({
                        id: $(this).val(),
                        amount: $(this).closest("tr").find(".amount-field").val(),
                        transaction: $(this).data('transaction'),
                    });
                    totalAmount += parseFloat($(this).closest("tr").find(".amount-field").val());
                });
                ...
                if (selectedBills.length > 0) {
                    ...
                    $.each(selectedBills, function (index, request) {
