...
@section('content')
    <div class="container-fluid">
        ...
        @if ($paymentVoucher->getApprovalModule())
            <div class="card mt-4">
                <div class="card-header">
                    <h4 class="mb-0">Approval Status</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>Current Status:</strong>
                                <span
                                    class="badge badge-{{ $paymentVoucher->approval_status === 'approved' ? 'success' : ($paymentVoucher->approval_status === 'rejected' ? 'danger' : 'warning') }}">
                                    {{ ucfirst($paymentVoucher->approval_status) }}
                                </span>
                            </p>

                            @if ($paymentVoucher->canApprove())
                                <form
                                    action="{{ route('approval.approve', ['model' => 'PaymentVoucher', 'id' => $paymentVoucher->id]) }}"
                                    method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <label for="approval_comments">Comments</label>
                                        <textarea name="comments" id="approval_comments" class="form-control" rows="2"></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-success">Approve</button>
                                </form>

                                <form
                                    action="{{ route('approval.reject', ['model' => 'PaymentVoucher', 'id' => $paymentVoucher->id]) }}"
                                    method="POST" class="mt-2">
                                    @csrf
                                    <div class="form-group">
                                        <label for="rejection_comments">Comments</label>
                                        <textarea name="comments" id="rejection_comments" class="form-control" rows="2"></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-danger">Reject</button>
                                </form>
                            @elseif($paymentVoucher->getApprovalStatus() === 'pending')
                                <div class="alert alert-info">
                                    @if ($paymentVoucher->getApprovalModule()->requires_sequential_approval)
                                        Waiting for previous approval steps to complete before you can approve.
                                    @else
                                        You have either already approved or don't have permission to approve this voucher.
                                    @endif
                                </div>
                            @endif
                        </div>
                        <div class="col-md-6">
                            ...
                            @php
                                $requiredApprovals = $paymentVoucher->getRequiredApprovals();
                                $currentApprovals = $paymentVoucher->getCurrentApprovals();
                                $approvalLogs = $paymentVoucher->approvalLogs()->with('user', 'role')->latest()->get();
                                $module = $paymentVoucher->getApprovalModule();
                            @endphp
                            ...
                            <ul class="list-group">
                                @foreach ($module->roles as $moduleRole)
                                    @php
                                        ...
                                        // For sequential approval, check if previous steps are complete
                                        $isActive = true;
                                        if ($module->requires_sequential_approval) {
                                            $previousRoles = $module->roles->where(
                                                'approval_order',
                                                '<',
                                                $moduleRole->approval_order,
                                            );
                                            foreach ($previousRoles as $prevRole) {
                                                if (
                                                    ($currentApprovals[$prevRole->role_id] ?? 0) <
                                                    $prevRole->approval_count
                                                ) {
                                                    $isActive = false;
                                                    break;
                                                }
                                ...
                                {{ $isComplete ? 'list-group-item-success' : '' }}">
                                        <div>
                                            {{ $role->name }}
                                            @if (!$isActive && $module->requires_sequential_approval)
                                                <small class="text-muted d-block">Waiting for previous approvals</small>
                                            @endif
                                        </div>
                                        <span
                                            class="badge badge-primary badge-pill">{{ $currentCount }}/{{ $requiredCount }}</span>
                                    </li>
                                @endforeach
                            </ul>

                            @if ($approvalLogs->isNotEmpty())
                                <h5 class="mt-3">Approval History</h5>
                                <div class="table-responsive">
                                        ...
                                        <tbody>
                                            @foreach ($approvalLogs as $log)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge badge-{{ $log->action === 'approved' ? 'success' : 'danger' }}">
                                                            {{ ucfirst($log->action) }}
                                                        </span>
                                                    </td>
                                                    <td>{{ $log->created_at->format('Y-m-d H:i') }}</td>
                                                    <td>{{ $log->comments ?? 'N/A' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
