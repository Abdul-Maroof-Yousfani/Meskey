...
@section('title')
    Edit Direct Payment Voucher - {{ $payment_voucher->unique_no }}
@endsection
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Edit Direct Payment Voucher</h4>
                </div>
                <div class="card-body">
                    <form id="ajaxSubmit" action="{{ route('direct.payment-voucher.update', $payment_voucher) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="voucher_type" id="voucher_type" class="form-control select2" required>
                                        ...
                                        <option value="bank_payment_voucher" {{ $payment_voucher->voucher_type === 'bank_payment_voucher' ? 'selected' : '' }}>
                                            Bank Payment Voucher
                                        </option>
                                        <option value="cash_payment_voucher" {{ $payment_voucher->voucher_type === 'cash_payment_voucher' ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="pv_date" id="pv_date" class="form-control"
                                        value="{{ $payment_voucher->pv_date->format('Y-m-d') }}" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="unique_no" id="unique_no" class="form-control"
                                        value="{{ $payment_voucher->unique_no }}" readonly>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="ref_bill_no" id="ref_bill_no" class="form-control"
                                        value="{{ $payment_voucher->ref_bill_no }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="bill_date" id="bill_date" class="form-control"
                                        value="{{ $payment_voucher->bill_date?->format('Y-m-d') }}">
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="cheque_no" id="cheque_no" class="form-control"
                                            value="{{ $payment_voucher->cheque_no }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="cheque_date" id="cheque_date" class="form-control"
                                            value="{{ $payment_voucher->cheque_date?->format('Y-m-d') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table class="table table-bordered" id="voucherEntriesTable">
                                        ...
                                        <tbody id="voucherEntriesBody">
                                            @forelse($items as $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="number" name="tax_amount[]" class="form-control tax-amount-input" step="0.01"
                                                            value="{{ $item->tax_amount }}" readonly>
                                                    </td>
                                                    <td>
                                                        <input type="number" name="net_amount[]" class="form-control net-amount-input" step="0.01"
                                            ...
                                            @empty
                                                <tr>
                                                    ...
                                                    <td><input type="number" name="amount[]" class="form-control amount-input" step="0.01" placeholder="0.00" required></td>
                                                    <td>
                                                        <select name="tax_id[]" class="form-control select2 tax-select">
                                                            <option value="">No Tax</option>
                                                            @foreach ($taxes as $tax)
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td><strong id="totalNetAmount">{{ $payment_voucher->total_amount ?? '0.00' }}</strong></td>
                                                <td colspan="2"></td>
                                            </tr>
                                            <tr>
                                                <td colspan="7">
                        ...
                        <div class="form-group text-right mt-4">
                            <button type="submit" class="btn btn-primary">Update Direct Payment Voucher</button>
                            <a href="{{ route('payment-voucher.index') }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    const currentAccountId = {{ $payment_voucher->account_id ?? 'null' }};
    ...
    function loadPvNumberAndAccounts() {
        ...
        if (!voucherType || !pvDate) {
            $('#account_id').empty().append('<option value="">Select Voucher Type & Date First</option>').trigger('change');
            return;
        }
        ...
        $.post('{{ route("payment-voucher.generate-pv-number") }}', {
            ...
            if (resp.success && resp.accounts) {
                ...
                if (!foundCurrent && currentAccountId !== null) {
                    $select.val('').trigger('change');
                } else {
                    $select.trigger('change');
                }
            } else {
                $('#account_id').empty().append('<option value="">No accounts available</option>').trigger('change');
            }
        }).fail(function() {
            alert('Error loading accounts.');
            $('#account_id').empty().append('<option value="">Error loading</option>').trigger('change');
        });
    }
    ...
    function calculateTotals() {
        ...
        $('#voucherEntriesBody tr').each(function() {
            const net = parseFloat($(this).find('.net-amount-input').val()) || 0;
            totalNet += net;
        });
    ...
    // Bind addRow only ONCE to prevent multiple rows
