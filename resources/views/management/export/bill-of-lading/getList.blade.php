    ...
    <tbody>
        @forelse ($billOfLadings as $key => $billOfLading)
            <tr>
                ...
                <td>{{ $billOfLading->bill_date ? \Carbon\Carbon::parse($billOfLading->bill_date)->format('d/m/Y') : 'N/A' }}</td>
                <td>{{ $billOfLading->snapshot_data['delivery_challan_no'] ?? ($billOfLading->exportDeliveryChallan->dc_no ?? 'N/A') }}</td>
                <td>{{ $billOfLading->snapshot_data['delivery_order_no'] ?? ($billOfLading->deliveryOrder->reference_no ?? 'N/A') }}</td>
                <td>{{ $billOfLading->snapshot_data['consignee_name'] ?? ($billOfLading->exportDeliveryChallan->customer->name ?? 'N/A') }}</td>
