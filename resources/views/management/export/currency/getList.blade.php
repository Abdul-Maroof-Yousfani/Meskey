        ...
        @if (count($currencies) != 0)
            @foreach ($currencies as $key => $currency)
                <tr>
                    ...
                    <td>
                        @canAccess('currency-edit')
                        <a class="info p-1 text-center position-relative "
                            onclick="openModal(this,'{{ route('currency.edit', $currency->id) }}','Edit Currency')">
                            <i class="ft-edit font-medium-3"></i></a>
                        @endcanAccess
                        @canAccess('currency-show')
                        <a class="info p-1 text-center position-relative "
                            onclick="openModal(this,'{{ route('currency.show', $currency->id) }}','Show Currency')">
                            <i class="ft-eye font-medium-3"></i></a>
                        @endcanAccess
                        @canAccess('currency-delete')
                        <a onclick="deletemodal('{{ route('currency.destroy', $currency->id) }}','{{ route('get.currency') }}')"
