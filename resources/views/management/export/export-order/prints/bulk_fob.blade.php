...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="data-section">
            <p>{{ $currencyCode }} {{ number_format((float) $avgRate, 2) }} PMT {{ strtoupper($exportOrder->incoterm->name ?? '') }} {{ strtoupper($exportOrder->portOfLoading->name ?? '') }} / KARACHI PORT, PAKISTAN</p>
            <p>TOTAL: {{ $currencyCode }} {{ number_format((float) $totalAmount, 2) }}</p>
        </div>

        <div class="section-title">ORIGIN:</div>
        <div class="data-section">
            <p>{{ strtoupper($exportOrder->originCountry->name ?? '') }}</p>
        </div>
        ...
        <div class="section-title">LOADING TERMS:</div>
        <div class="data-section">
            <p>BUYER TO GIVE MINIMUM 7 (SEVEN) DAYS WRITTEN PRE-ADVISE OF VESSEL'S ETA.</p>
            ...
            <p style="margin-top: 10px;">SELLER TO LOAD AT THE MINIMUM DAILY RATE OF {{ number_format((float) $exportOrder->minimum_daily_rate) }} METRIC TONS BASIS FOUR (4) WORKABLE GEARS OR PRORATA IF LESS PER WEATHER WORKING DAY OF 24 (TWENTY-FOUR) CONSECUTIVE HOURS SATURDAY NOON ONWARDS AND SUNDAYS, HOLIDAY EXCLUDED, EVEN IF USED ({{ strtoupper($exportOrder->minimum_daily_shex_eiu) }} {{ strtoupper($exportOrder->load_term_type ?? 'SHEX EIU') }}).</p>
        ...
        <div class="data-section">
            <p>{{ strtoupper($exportOrder->modeOfTerm->name ?? '') }}</p>
        </div>
