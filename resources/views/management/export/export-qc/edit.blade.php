<form action="{{ route('export-qc.update', $ExportQc->id) }}" method="POST" id="ajaxSubmit" autocomplete="off" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <input type="hidden" id="listRefresh" value="{{ route('get.export-qc') }}" />
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="loading_program_item_id" id="loading_program_item_id">
                    ...
                    @foreach ($Tickets as $ticket)
                        <option value="{{ $ticket->id }}" {{ $ticket->id == $ExportQc->loading_program_item_id ? 'selected' : '' }}>
    ...
    <div class="row" id="ticketDataContainer" style="margin-left: 4px; margin-right: 4px; width: 100%;">
        @if(count($Orders) > 0)
            ...
            <input type="hidden" name="customer" value="{{ $ExportQc->customer }}" />
            <input type="hidden" name="commodity" value="{{ $ExportQc->commodity }}" />
            <input type="hidden" name="so_qty" value="{{ $ExportQc->so_qty }}" />
            <input type="hidden" name="do_qty" value="{{ $ExportQc->do_qty }}" />
            <input type="hidden" name="factory" value="{{ $ExportQc->factory }}" />
            <input type="hidden" name="gala" value="{{ $ExportQc->gala }}" />
        @else
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <textarea name="qc_remarks" placeholder="Enter QC remarks" class="form-control" rows="3">{{ $ExportQc->qc_remarks }}</textarea>
            </div>
        </div>
    </div>
        ...
        <div class="col-xs-12 col-sm-6 col-md-6">
            <div class="form-group">
                ...
                <select class="form-control" name="status">
                    ...
                    <option value="accept" {{ $ExportQc->status == 'accept' ? 'selected' : '' }}>Accept</option>
                    <option value="reject" {{ $ExportQc->status == 'reject' ? 'selected' : '' }}>Reject</option>
                </select>
            </div>
        </div>
        <div class="col-xs-12 col-sm-6 col-md-6">
    ...
    <div class="row bottom-button-bar">
        <div class="col-12">
            <a href="{{ route('export-qc.index') }}" class="btn btn-secondary">Cancel</a>
    ...
    $(document).ready(function() {
        $('#loading_program_item_id').change(function() {
            ...
            if (loading_program_item_id) {
                $.ajax({
                    url: '{{ route('export.getTicketRelatedDataForQc') }}',
                    type: 'GET',
                    data: {
