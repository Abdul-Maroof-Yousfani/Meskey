<form action="{{ route('export-second-weighbridge.update', $SecondWeighbridge->id) }}" method="POST" id="ajaxSubmit" autocomplete="off">
    @csrf
    @method('PUT')
    <input type="hidden" id="listRefresh" value="{{ route('get.export-second-weighbridge') }}" />
    ...
    <div class="row" id="slabsContainer">
        @if($SecondWeighbridge->loadingSlip)
            @include('management.export.second-weighbridge.getSecondWeighbridgeRelatedData', [
...
<script>
    ...
    $(document).ready(function() {
        $('#loading_slip_id').change(function() {
            ...
            if (loading_slip_id) {
                $.ajax({
                    url: '{{ route('export.getSecondWeighbridgeRelatedData') }}',
                    type: 'GET',
                    data: {
