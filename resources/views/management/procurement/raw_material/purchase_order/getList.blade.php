<table class="table m-0">
    <thead>
        <tr>
            ...
            <th class="col-sm-2">Quantity Details</th>
                    ...
                    <td>#{{ $row->contract_no }} <br> {{ $row->product->name ?? ' N/A' }}
                        ...
                        <div>
                            <span class="badge badge-primary mt-2">
                                {{ formatEnumValue($row->calculation_type ?? 'N/A') }}
                            </span>
                            <span class="badge badge-{{ $row->status == 'completed' ? 'success' : 'warning' }}">
                                {{ formatEnumValue($row->status == 'draft' ? 'Pending' : $row->status) }}
                            </span>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="div-box-b">
                            <small>
                                ...
                                {{ ($row->min_quantity ?? '-') . ' - ' . ($row->max_quantity ?? '-') }} <br>
                                <strong>Arrived Qty:</strong>
                                {{ $row->totalArrivedNetWeight->total_arrived_net_weight ?? 0 }} <br>
                                ...
                                <br>
                                <strong>Truck Ordered:</strong> {{ $row->no_of_trucks ?? 0 }} <br>
                                <strong>Trucks Arrived:</strong>
                                {{ $row->totalClosingTrucksQty->total_closing_trucks_qty ?? 0 }} <br>
                                <strong>Remaining Truck:</strong>
                                {{ ($row->no_of_trucks ?? 0) - ($row->totalClosingTrucksQty->total_closing_trucks_qty ?? 0) }}
                                <br>
                            </small>
                        </div>
                    </td>
