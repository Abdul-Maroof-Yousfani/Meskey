                ...
                <table class="table table-bordered" id="purchaseRequestTable">
                    ...
                    <tbody id="purchaseRequestBody">
                        @forelse ($PurchaseQuotationData ?? [] as $key => $data)
                            @php
                                $alreadyApprovedQty = $data->purchase_request?->purchase_quotation_data()
                                    ->where('am_approval_status', 'approved')
                                    ->sum('qty') ?? 0;
                            @endphp
                            <tr id="row_{{ $key }}" 
                                data-pr-data-id="{{ $data->purchase_request_data_id }}"
                                data-pr-qty="{{ $data->purchase_request->qty ?? 0 }}"
                                data-already-approved="{{ $alreadyApprovedQty }}"
                                ...
                                <td style="width: 30%">
                                    <input type="date" value="{{ $data->delivery_date }}" class="form-control" readonly disabled>
                                </td>
...
<script>
    ...
    // ✅ Initialize visibility on load for Comparison
