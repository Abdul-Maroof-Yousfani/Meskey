{{-- resources/views/management/procurement/store/sales_inquiry/getList.blade.php --}}
            ...
            <table class="table table-hover m-0">
                ...
                <tbody>
                    @forelse($groupedSalesReturns as $group)
                        ...
                        @foreach($group['items'] as $itemRow)
                            <tr>
                                ...
                                <td class="text-right align-middle">
                                    {{ number_format($itemRow['item_data']->quantity ?? 0, 2) }}
                                    <small class="text-muted">{{ $itemRow['item']->unitOfMeasure->name ?? '' }}</small>
                                </td>

                                <td class="text-right align-middle">
                                    {{ number_format($itemRow['si_data']->rate, 2) }}
                                </td>

                                <td class="text-right align-middle">
                                    {{ number_format($itemRow["si_data"]->rate * $itemRow["si_data"]->qty, 2) }}
                                </td>
                                    ...
                                    <td rowspan="{{ $group['rowspan'] }}" class="text-center align-middle">
                                        <div class="btn-group" role="group">
                                            ...
                                            @if(auth()->user()->id == $group['created_by_id'])
                                                ...
                                                <button 
                                                    onclick="openModal(this,'{{ route('sales.sales-return.edit', ['sales_return' => $group['id']]) }}','Edit Sale Return', false, '100%')"
                                                    class="btn btn-sm btn-warning" title="Edit" style="margin-right: 10px;">
                                                    <i class="ft-edit"></i>
                                                </button>

                                                
                                            <button onclick="deletemodal('{{ route('sales.sales-return.destroy', ['sales_return' => $group['id']]) }}', '{{ route('sales.get.sales-return.list') }}')" type="button"
