...
<!-- <form action="" method="POST" id="ajaxSubmit" autocomplete="off"> -->
    ...
    @if(in_array(strtolower($delivery_order->am_approval_status ?? ''), ['approved', 'rejected']))
        @php
            $isApproved = strtolower($delivery_order->am_approval_status) === 'approved';
        ...
        <div class="alert {{ $alertClass }} px-3 py-2 mt-2 d-flex align-items-center justify-content-between" style="border-radius: 6px;">
            <div>
                ...
                <strong>Status: {{ ucfirst($delivery_order->am_approval_status) }}</strong> - This Delivery Order has been finalized. Its status cannot be changed.
            </div>
        </div>
    @elseif(strtolower($delivery_order->am_approval_status ?? '') === 'reverted')
        <div class="alert alert-warning px-3 py-2 mt-2 d-flex align-items-center justify-content-between" style="border-radius: 6px;">
            <div>
                ...
                <strong>Status: Reverted</strong> - This Delivery Order has been reverted. It cannot be approved or rejected until changes are made and resubmitted to pending.
            </div>
...
@php
    $doModule = $delivery_order->getApprovalModule();
    $doApprovalLogs = $doModule ? \App\Models\ApprovalsModule\ApprovalLog::where('record_id', $delivery_order->id)->where('module_id', $doModule->id)->with(['user', 'role'])->orderBy('created_at', 'desc')->get() : collect();
@endphp
    ...
    <div class="approval-table-wrapper" style="margin-top: 25px; padding-bottom: 10px !important;">
        <div class="card border" style="box-shadow: none; margin-bottom: 0 !important;">
            <div class="card-header bg-light py-2 d-flex justify-content-between align-items-center">
                ...
                <span class="badge badge-info">{{ $doApprovalLogs->count() }} {{ \Illuminate\Support\Str::plural('Action', $doApprovalLogs->count()) }}</span>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover mb-0" style="font-size: 13px;">
                        ...
                        <tbody>
                            @foreach ($doApprovalLogs as $index => $log)
                                @php
