@foreach ($loading_programs as $index => $loading_program_item)
    ...
    <tr id="row_{{ $index }}">
        ...
        <td>
            <input type="text" name="no_of_bags[]" id="no_of_bags_{{ $index }}" value="{{ $loading_slip->no_of_bags ?? 0 }}"
        ...
        <td>
            <input type="text" name="qty[]" id="qty_{{ $index }}" value="{{ round($net_weight) }}" {{--
                onkeyup="check_balance(this, 'no_of_bags_{{ $index }}')" --}} {{--
                data-balance="{{ delivery_challan_balance($data->id) }}" --}} class="form-control qty" step="0.01" min="0"
