...
@if(in_array(strtolower($delivery_challan->am_approval_status ?? ''), ['approved', 'rejected']))
    @php
        $isApproved = strtolower($delivery_challan->am_approval_status) === 'approved';
    ...
    <div class="alert {{ $alertClass }} px-3 py-2 mt-2 d-flex align-items-center justify-content-between" style="border-radius: 6px;">
        <div>
            ...
            <strong>Status: {{ ucfirst($delivery_challan->am_approval_status) }}</strong> - This Delivery Challan has been finalized. Its status cannot be changed.
        </div>
    </div>
@elseif(strtolower($delivery_challan->am_approval_status ?? '') === 'reverted')
    <div class="alert alert-warning px-3 py-2 mt-2 d-flex align-items-center justify-content-between" style="border-radius: 6px;">
        <div>
            ...
            <strong>Status: Reverted</strong> - This Delivery Challan has been reverted. It cannot be approved or rejected until changes are made and resubmitted to pending.
        </div>
