...
@foreach ($inquiry->sales_inquiry_data as $data)
    ...
    <tr id="row_{{ $i }}">
        <td>
            ...
            <input type="text" value="{{ getItem($data->item_id)?->name ?? '' }}" value="{{ round($data->qty) }}"
        ...
        <td>
            <input type="number" name="minimum_qty[]" id="minimum_qty_{{ $i }}" value="{{ round($data->minimum_qty) }}" class="form-control minimum_qty" step="0.01" min="0">
        </td>
        <td>
            <input type="number" name="qty[]" id="qty_{{ $i }}" onkeyup="calc(this)"
                value="{{ round($data->qty) }}" class="form-control qty" step="0.01" min="0" readonly>
        </td>
        <td>
            <input type="number" name="rate[]" id="rate_{{ $i }}" onkeyup="calc(this)"
                value="{{ $data->rate }}" class="form-control rate rate_per_kg" step="0.01" min="0" readonly>
        </td>
        <td>
