    ...
    @slot('body')
        ...
        @foreach ($tickets as $row)
            @php
                ...
                $weighbridgeAmount = 0;
                if ($isWeighed) {
                    $weighbridgeAmount = getTruckWeighbridgeAmount($row->truck_type_id, $row->location_id) ?: ($row->truckType?->weighbridge_amount ?? 0);
                }

                $sampleAmount = $row->sample_money ?? $row->truckType?->sample_money ?? 0;
            ...
            <tr>
                ...
                <td>{{ $row->firstWeighbridge?->weight ? number_format((float)$row->firstWeighbridge->weight, 0, '.', '') : '' }}</td>
                <td>{{ $row->secondWeighbridge?->weight ? number_format((float)$row->secondWeighbridge->weight, 0, '.', '') : '' }}</td>
