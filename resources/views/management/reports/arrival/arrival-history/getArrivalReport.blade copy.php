<x-sticky-table :items="$tickets" :leftSticky="2" :rightSticky="1" :emptyMessage="'No tickets found'"
    ...
    <tr
        ...
        <td data-slaptypename="{{ $deductionValueSlabinitial[$slab->id]['name'] ?? 'N/A' }}">
            @if(isset($deductionValueSlabinitial[$slab->id]['checklist_value']) &&
            $deductionValueSlabinitial[$slab->id]['checklist_value'] != 0)
        ...
        <td data-slaptypename="{{ $deductionValueSlabInner[$slab->id]['name'] ?? 'N/A' }}">
            @if(isset($deductionValueSlabInner[$slab->id]['checklist_value']) &&
            $deductionValueSlabInner[$slab->id]['checklist_value'] != 0)
            {{-- {{ $deductionValueSlab[$slab->id]['deduction'] }}
            {{ $deductionValueSlab[$slab->id]['unit'] ?? '' }} --}}
        ...
        <td>
            <button class="info p-1 text-center mr-2 position-relative btn" @disabled(!$row->freight ||
        ...
        <td>
            <button class="info p-1 text-center mr-2 position-relative btn" @disabled(!$row->arrivalSlip)
                ...
                @else
                onclick="openModal(this,'{{ route('arrival-slip.edit', $row->arrivalSlip->id) }}','Ticket: {{
                $row->unique_no }}', true, '100%')" @endif>
                <a href="#">
                    <i class="ft-eye font-medium-3"></i>
                </a>
            </button>
