        ...
        @if (count($stitchings) != 0)
            @foreach ($stitchings as $key => $row)
                <tr>
                    ...
                    <td>
                        @can('role-edit')
                            <a onclick="openModal(this,'{{ route('stitching.edit', $row->id) }}','Edit Stitching')"
                                class="info p-1 text-center mr-2 position-relative ">
                                <i class="ft-edit font-medium-3"></i>
                            </a>
                        @endcan
                        @can('role-delete')
                            <a onclick="deletemodal('{{ route('stitching.destroy', $row->id) }}','{{ route('get.stitching') }}')"
