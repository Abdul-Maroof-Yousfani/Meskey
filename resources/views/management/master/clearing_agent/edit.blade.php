<form action="{{ route('clearing-agent.update', $clearingAgent->id) }}" method="POST" id="ajaxSubmit"
    autocomplete="off" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <input type="hidden" id="listRefresh" value="{{ route('get.clearing-agent') }}" />

    <div class="row">
        <div class="col-12">
            <h6 class="header-heading-sepration">
                Clearing Agent Detail
            </h6>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-8">
            <div class="form-group">
                <label><span class="text-danger required-asterisk">*</span> Clearing Agent Name:</label>
                <input type="text" name="name" value="{{ old('name', $clearingAgent->name) }}"
                    placeholder="Clearing Agent Name" class="form-control" autocomplete="off" />
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-4">
            <div class="form-group">
                <label><span class="text-danger required-asterisk">*</span> Rate:</label>
                <input type="number" name="rate" value="{{ old('rate', $clearingAgent->rate) }}"
                    placeholder="Rate" class="form-control" step="0.01" min="0" autocomplete="off" />
            </div>
        </div>
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                <label>NTN#: <small>(Optional)</small></label>
                <input type="text" name="ntn" value="{{ old('ntn', $clearingAgent->ntn) }}" placeholder="NTN No"
        ...
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                ...
                <input type="text" name="stn" value="{{ old('stn', $clearingAgent->stn) }}" placeholder="STN No"
                    class="form-control" />
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div id="card-container" class="mb-4">
                @if (count($clearingAgent->companyBankDetails) > 0)
                    @foreach ($clearingAgent->companyBankDetails as $index => $bank)
                ...
                <select name="account_id" class="form-control">
                    ...
                    @foreach ($accounts as $account)
                        <option value="{{ $account->id }}"
                            {{ $clearingAgent->account_id == $account->id ? 'selected' : '' }}>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <label><span class="text-danger required-asterisk">*</span>Owner Name:</label>
                <input type="text" name="owner_name" value="{{ old('owner_name', $clearingAgent->owner_name) }}"
                    placeholder="Owner Name" class="form-control" autocomplete="off" />
            </div>
        </div>
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                <label><span class="text-danger required-asterisk">*</span>Owner Mobile No:</label>
                <input type="text" name="owner_mobile_no" placeholder="03001234567"
                    value="{{ old('owner_mobile_no', $clearingAgent->owner_mobile_no) }}" class="form-control"
        ...
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                <label><span class="text-danger required-asterisk">*</span>Owner CNIC:</label>
                <input type="text" name="owner_cnic_no" placeholder="12345-1234567-1"
                    value="{{ old('owner_cnic_no', $clearingAgent->owner_cnic_no) }}" class="form-control cnic-input"
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div id="card-container2" class="mb-4">
                @if (count($clearingAgent->ownerBankDetails) > 0)
                    @foreach ($clearingAgent->ownerBankDetails as $index => $bank)
    ...
    <div class="row">
        ...
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                <label>Name:</label>
                <input type="text" name="next_to_kin" value="{{ old('next_to_kin', $clearingAgent->next_to_kin) }}"
        ...
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                ...
                <input type="text" name="next_to_kin_mobile_no" placeholder="03001234567" class="form-control"
                    value="{{ old('next_to_kin_mobile_no', $clearingAgent->next_to_kin_mobile_no) }}"
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <select name="status" class="form-control">
                    <option value="active" {{ old('status', $clearingAgent->status) == 'active' ? 'selected' : '' }}>
                        Active</option>
                    <option value="inactive" {{ old('status', $clearingAgent->status) == 'inactive' ? 'selected' : '' }}>
                        Inactive</option>
                </select>
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <label>Address:</label>
                <textarea name="address" rows="2" class="form-control" placeholder="Clearing Agent Address">{{ old('address', $clearingAgent->address) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="row bottom-button-bar">
        <div class="col-12">
            <a href="{{ route('clearing-agent.index') }}"
