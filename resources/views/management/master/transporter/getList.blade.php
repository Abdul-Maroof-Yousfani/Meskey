<table class="table m-0">
    <thead>
        <tr>
            ...
            <th class="col-sm-2">Transporter </th>
    ...
    <tbody>
        @if (count($Transporters) != 0)
            @foreach ($Transporters as $key => $row)
                <tr>
                    <td>
                        <p class="m-0">
                            {{ $row->unique_no }}
                    ...
                    <td>
                        @can('role-edit')
                            <a onclick="openModal(this,'{{ route('transporter.edit', $row->id) }}','Edit Transporter')"
                                class="info p-1 text-center mr-2 position-relative ">
                                <i class="ft-edit font-medium-3"></i>
                            </a>
                        @endcan
                        @can('role-delete')
                            <a onclick="deletemodal('{{ route('transporter.destroy', $row->id) }}','{{ route('get.transporter') }}')"
